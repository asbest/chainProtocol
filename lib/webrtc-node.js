...
const SteamSignaling = require('./steam-signaling');
...
class P2PNode {
    constructor(blockchain, opts = {}) {
        ...
        this.peers = [];
        this.createPeer = opts.createPeer || ((opts) => new SimplePeer({
            ...opts,
            wrtc: wrtc
        }));
    }
    ...
    async hostViaSteam(appId, onConnect) {
      this.steamSignaling = new SteamSignaling(appId);
      const lobbyId = await this.steamSignaling.createLobby();
      console.log(`Steam lobby created with ID: ${lobbyId}`);

      this.steamSignaling.setOnMessage((user, message) => {
        if (message.type === 'offer') {
          ...
          peer.on('signal', (answer) => {
            this.steamSignaling.sendMessage(user, { type: 'answer', payload: answer });
          });

          peer.on('connect', () => {
            console.log(`Connected to peer ${user}`);
            this.peers.push(peer);
            this.setupPeerEvents(peer);
            if (onConnect) onConnect(peer);
            this.syncChain(peer);
    ...
    async joinViaSteam(appId, lobbyId) {
      return new Promise(async (resolve, reject) => {
        this.steamSignaling = new SteamSignaling(appId);
        await this.steamSignaling.joinLobby(lobbyId);
        ...
        peer.on('signal', (offer) => {
          const lobbyOwner = this.steamSignaling.client.matchmaking.getLobbyOwner(lobbyId);
          this.steamSignaling.sendMessage(lobbyOwner, { type: 'offer', payload: offer });
        });
